...
class Experiment(CourseObject):
    ...
    def get_is_finalized(cls, course_id):
        """ TODO: remove in second phase of refactor""" 
        return cls.get_placeholder_course_experiment(course_id).tracks_finalized
    
    @classmethod
    def set_finalized(cls, course_id):
        """ TODO: remove in second phase of refactor""" 
        cls.get_placeholder_course_experiment(course_id).update(tracks_finalized=True)


class Track(CourseObject):
    name = models.CharField(max_length=256)
    notes = models.CharField(max_length=1024)
    experiment = models.ForeignKey(Experiment, related_name="tracks", null=True)
...
class TrackProbabilityWeight(CourseObject):
    ...
    experiment = models.ForeignKey(Experiment, related_name="track_probabilites", null=True)
...
class InterventionPoint(CourseObject):
    ...
    name = models.CharField(max_length=256)
    notes = models.CharField(max_length=1024)
    experiment = models.ForeignKey(Experiment, related_name="intervention_points", null=True)
...
#TODO: remove in second phase of refactor
CourseStudent = ExperimentStudent
CourseSettings = Experiment